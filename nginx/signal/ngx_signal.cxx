...
/*
您提出的疑问是：
在函数 ngx_init_signals() 中，定义了局部变量 struct sigaction sa;，它位于栈区。
函数执行完毕后，sa 变量会被销毁（因为它是一个局部变量）。
既然 sa 被销毁了，那么通过 sa 配置的信号处理动作是否也会失效？
为什么这样的代码仍然是有效的，信号处理动作依然生效？
回答概述
sigaction() 函数的作用：当您调用 sigaction() 时，您传入的 sa 结构体的内容会被内核读取，并将信号处理动作注册到内核的数据结构中。这个过程完成后，sa 变量本身的生命周期和作用域就不再重要。

sa 的生命周期：sa 是局部变量，其生命周期在函数结束时结束。但是信号处理动作已经被内核记录下来，不再依赖于 sa 变量。

为什么有效：因为 sigaction() 是一个系统调用，它将 sa 中的信息复制到内核空间，信号处理动作的注册与 sa 变量的生存期无关。*/
